...
**支援私有化部署，專為重視資料主權的企業打造的電子簽名解決方案。**
點點簽 DottedSign Self-Hosted 提供企業在內部網路環境中自主部署點點簽系統的選項。此專案旨在協助有合規需求或封閉網路環境限制的組織，能更彈性地建置電子簽名流程。

## 核心特色

- 資料本地化儲存：系統設計為可於內網環境中獨立運行，讓文件與簽署紀錄留存於企業私有伺服器內
...
1. 解鎖進階安全模組：如具備法律效力的文件憑證、Email OTP、簡訊 OTP 等功能
...
- 若您的組織僅於內部安裝使用，且未修改原始碼，將不受開源條款的強制影響
- 若您修改了系統原始碼（例如調整介面），並將修改後的系統對外提供服務予第三方，則依約定必須將修改後的原始碼開源。如有不開源的客製化需求，建議洽詢商業授權

## 技術支援 (Support)
- 開源版技術討論：如遇部署技術問題，歡迎於 [GitHub Issues](https://github.com/DottedSign-Official/dottedsign-self-hosted/issues) 提出，由社群共同交流與協助。

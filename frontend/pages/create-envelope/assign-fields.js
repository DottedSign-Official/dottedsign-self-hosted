...
import SwitchFile from "../../containers/SwitchFile";
...
const DocumentDetails = () => {
  ...
  const { fileUrl, assignes, envelopeName } = useSelector(
    (state) => state.create,
  );
  ...
  useEffect(() => {
    const envelopeId = Router.query.envelopeId;

    if (!envelopeId && (!assignes || assignes.length < 1)) {
  ...
  useEffect(() => {
    ...
    if (envelopeId) {
      // NOTE: draft
      dispatch(fetchDraft({ envelope_id: envelopeId }));
    }
  }, [dispatch, Router]);
  ...
  return (
    <PageWrapper backcolor="#EEEFF3">
      ...
      <CoverDownloadApp>
        <Menu page="details-c-n-i" isDisabled={!isRenderDone} />
        <WrapperFull>

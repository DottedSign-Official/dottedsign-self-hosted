...
import RenameTemplate from "../../../../components/Modal/Types/TemplateRename";
...
const NEW_TEMPLATE_NAME = "NEW_TEMPLATE_NAME";

describe("TemplateRename", () => {
  mockReduxBeforeAll();

  test("Submit button should not in document when isLoading is true.", async () => {
    mockRedux({ template: { isLoading: true } });
    const { screen } = setup(<RenameTemplate {...props} />);
  ...
  test("Click cancel button should call onModalClose.", async () => {
    ...
    const { screen, user } = setup(
      <RenameTemplate onModalClose={onModalClose} {...props} />,
    );
  ...
  test("Type input should update input's value.", async () => {
    mockRedux({ template: { isLoading: false } });
    const { screen, user } = setup(<RenameTemplate {...props} />);

    const input = screen.getByRole("textbox");
    expect(input).toHaveValue(props.data.templateName);

    input.focus();
    await user.keyboard(CTRL_A_DELETE);
    await user.keyboard(NEW_TEMPLATE_NAME);

    expect(input).toHaveValue(NEW_TEMPLATE_NAME);
  });

  test("Type and submit should dispatch redux with new name.", async () => {
    ...
    expect(dispatchMock.mock.calls[0][0]).toMatchObject({
      payload: {
        ...
        file_name: NEW_TEMPLATE_NAME,
      },
    });
  });

  test("Submit should not dispatch when input is empty.", async () => {
    const dispatchMock = mockRedux({ template: { isLoading: false } });

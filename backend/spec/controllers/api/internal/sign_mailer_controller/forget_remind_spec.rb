...
RSpec.describe '/api/internal/sign_mail_controller#forget_remind', type: :request do
  include_context 'rpdoc'

  before(:each) do |example|
    example.metadata[:rpdoc_action_key] = 'forget_remind'
    example.metadata[:rpdoc_action_name] = '寄送忘記簽名提醒信'
    example.metadata[:rpdoc_example_folders] =  ['internal', 'mailer', 'sign_mailer', 'sign_mailer']

    @path = '/api/internal/mailer/forget_remind'
  ...
  it 'should return 200' do |example|
    ...
    example.metadata[:rpdoc_example_name] = '成功寄送忘記簽名提醒信'

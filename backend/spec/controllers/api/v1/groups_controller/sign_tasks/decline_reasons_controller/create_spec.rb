...
RSpec.describe '/api/v1/groups/sign_tasks/decline_reasons#create', type: :request do
  ...
  before(:each) do |example|
    ...
    @params = { group_id: @group.id, content: 'create group decline reason' }
    @path = '/api/v1/groups/sign_tasks/decline_reasons'
  end

  describe '#create' do
    it 'should return 200 if create decline reasons success', rpdoc_example_key: 200, rpdoc_example_name: '新增團隊拒簽原因成功' do
      post @path, params: @params.to_json, headers: @headers
      expect(response).to have_http_status(200)
      expect(json['data']['content']).to eq(@params[:content])
      expect(json['data']['system_reserved']).to be(false)
      expect(@group2.reload.decline_reasons.count).to be(0)
    end
    ...
    it 'should return 403_056 if group_member has no permission to create decline reason', rpdoc_example_key: 403_056, rpdoc_example_name: '新增團隊拒簽原因失敗 (403_056 member has no permission)' do

...
RSpec.describe Api::V1::SignTasks::AdminController, type: :request do
  ...
  before(:each) do |example|
    example.metadata[:rpdoc_action_key] = 'summary_csv'
    example.metadata[:rpdoc_action_name] = '匯出群組任務報表CSV'
  ...
  describe '#tasks' do
    before(:each) do
      @path = '/api/v1/sign_tasks/admin/summary_csv'
      @params = {
        group_id: @group.id,
        emails: [@member.email],
    ...
    it 'should return 200', rpdoc_example_key: 200, rpdoc_example_name: 'export admin report success' do
    ...
    it 'should return 400207 if member not belongs to group', rpdoc_example_key: 400207, rpdoc_example_name: 'export admin report failed (member not belongs to group)' do
    ...
    it 'should return 403056 if group member forbid to view tasks', rpdoc_example_key: 403056, rpdoc_example_name: 'export admin report failed (group member forbid to view tasks)' do
